...
using Contable.Migrations.Seed;
...
namespace Contable.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : ContableAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<ContableDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{ContableConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{ContableConsts.ConnectionStringName}", constring);
        }
