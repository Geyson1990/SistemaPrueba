...
namespace Contable.Authorization.Roles
{
    /// <summary>
    /// Role manager.
    ...
    public class RoleManager : AbpRoleManager<Role, User>
    {
        ...
        private void CheckPermissionsToUpdate(Role role, IEnumerable<Permission> permissions)
        {
            if(role.Name == StaticRoleNames.Host.Admin)
                throw new UserFriendlyException("Aviso", "No se pueden editar los permisos del administrador");
        }

        private new string L(string name)
        {
            return _localizationManager.GetString(ContableConsts.LocalizationSourceName, name);
        }
    }
}